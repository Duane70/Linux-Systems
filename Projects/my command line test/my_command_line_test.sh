...
function sign_up()
{
    ...
    username_arr=(`cut -d " " -f1 /home/duane/ECEP/LinuxSystem/project/usernames.txt`) #store all usernames in an array
...
username_arr=(`cut -d " " -f1 /home/duane/ECEP/LinuxSystem/project/usernames.txt`)  #storing the contents of file in an array
password_arr=(`cut -d " " -f1 /home/duane/ECEP/LinuxSystem/project/passwords.txt`)
...
{
    clear
    lines=`wc -l /home/duane/ECEP/LinuxSystem/project/question_bank.txt | cut -d " " -f1` 
   ...
   do
       answer_arr=(`cut -d " " -f1 /home/duane/ECEP/LinuxSystem/project/answers.txt`)      #storing content of file in an array
	  ...
	  echo Timeout >> ./test_data/answer_file.csv
	   ...
	   then
	       echo "Correct answer"
               echo "$ans is correct answer and you have answered in ${SECONDS}s" >> ./test_data/answer_file.csv
               echo "$ans is correct answer and you have answered in ${SECONDS}s" >> ./test_data/test_activity.log
	       ra=$(($ra+1))
	   
	   else
	       echo "Incorrect answer"
	       echo "$ans is wrong answer and you have answered in ${SECONDS}s" >> ./test_data/answer_file.csv
               echo "$ans is wrong answer and you have answered in ${SECONDS}s" >> ./test_data/test_activity.log
	   fi
        fi
